...
export default function Guardian() {
  ...
  useEffect(() => {
    ...
    // ⚙️ Smart alerts
    if (solar < 20 && hour >= 18) {
      newMessages.push("🌙 Night detected. Solar generation dropping.");
    }
    if (wind > 80) {
      newMessages.push("💨 Strong wind surge detected. Adjusting load balancing.");
    }
    if (wind < 10 && solar < 15 && battery < 40) {
      newMessages.push("🌬️ Low wind and solar input. Switching to grid backup.");
    }
    if (grid > 40 && battery > 80) {
      newMessages.push("🔌 Grid backup deactivated. Storage capacity restored.");
    }
    if (battery < 25 && solar < 10 && wind < 20) {
      newMessages.push("⚠️ Critical energy shortage. Prioritizing essential systems.");
    }
    ...
    if (newMessages.length > 0) {
      ...
      if (heartbeatTimer >= 7) {
        const stableMsg =
          risk === "HIGH"
            ? "⚠️ Guardian observing instability across energy sources..."
            : risk === "MEDIUM"
            ? "🟡 Systems balanced. Minor fluctuations in power mix."
            : "🟢 Energy flow stable across solar, wind, and grid.";
        setLogs((prev) => [
  ...
  }, [solar, wind, grid, battery, load, risk, hour, heartbeatTimer]);
  ...
  return (
    <section className="max-w-5xl mx-auto px-4 py-10 space-y-6">
      ...
      <div className="grid sm:grid-cols-4 gap-4">
        <Stat label="Solar" value={solar.toFixed(0)} unit="%" icon="☀️" />
        <Stat label="Wind" value={wind.toFixed(0)} unit="%" icon="💨" />
        <Stat label="Grid" value={grid.toFixed(0)} unit="%" icon="⚡" />
        <Stat label="Battery" value={battery.toFixed(0)} unit="%" icon="🔋" />
      </div>
...
function GuardianSummary({ battery, risk }) {
  const status =
    risk === "HIGH"
      ? "⚠️ Power instability detected — immediate load management engaged."
      : risk === "MEDIUM"
      ? "⚡ Stable operation with moderate fluctuations."
      : "✅ All systems synchronized. Energy network running optimally.";
